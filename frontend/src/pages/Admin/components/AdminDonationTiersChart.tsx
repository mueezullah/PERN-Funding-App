...
import { PieChart, DollarSign, Users, Award, ShieldAlert } from "lucide-react";
...
interface AdminDonationTiersChartProps {
  tiers: DonationTierItem[];
  totalRaised: number;
  totalDonationsCount: number;
}

const AdminDonationTiersChart: React.FC<AdminDonationTiersChartProps> = ({
  tiers = [],
  totalRaised = 0,
        ...
        <div>
          <div className="flex items-center gap-2">
            <CardTitle className="text-lg font-bold flex items-center gap-2 text-foreground">
              ...
              Platform Donation Tier Distribution
            </CardTitle>
          ...
          <CardDescription className="text-xs text-muted-foreground mt-1">
            Global ticket-size segmentation across all platform contributions
          </CardDescription>
        </div>

        <div className="text-right">
          <span className="text-xs font-semibold text-muted-foreground">Platform Total: </span>
      ...
      <CardContent>
        ...
        <div className="space-y-2 mb-6">
          <div className="flex items-center justify-between text-xs font-semibold text-muted-foreground">
            ...
            <span>100% Platform Backer Base</span>
          </div>
            ...
            {tiers.map((tier) => {
              ...
              return (
                <div
                  ...
                  title={`${tier.label}: ${tier.count} contributions (${tier.percentageOfDonations}%) - $${tier.totalAmount.toLocaleString()}`}
                />
              );
            })}
          </div>
            ...
            >
              <div className="flex items-center justify-between gap-2 mb-2">
                ...
                <Badge variant="secondary" className="text-[10px] font-bold py-0.5 px-2">
                  {tier.percentageOfDonations}% volume
                </Badge>
              </div>
              ...
              <div className="flex items-baseline justify-between pt-1">
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground flex items-center gap-1">
                    ...
                    {tier.percentageOfRevenue}% of platform total
                  </p>
                </div>
        ...
        <div className="mt-5 pt-3.5 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
          <div className="flex items-center gap-1.5">
            ...
            <span>Platform fee collection is optimized across higher tier distributions</span>
          </div>
          <span className="font-semibold text-foreground">
            Platform Avg: ${totalDonationsCount > 0 ? (totalRaised / totalDonationsCount).toFixed(2) : "0.00"}
          </span>
        </div>
      </CardContent>
    </Card>
...
export default AdminDonationTiersChart;
